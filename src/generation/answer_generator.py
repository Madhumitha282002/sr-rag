...
from src.generation.prompt_manager import REFUSAL_MESSAGE, build_prompt
...
def generate_answer(
    ...
    sources = [{**chunk, "citation_index": i + 1} for i, chunk in enumerate(included_chunks)]
    ...
    # Call LLM
    logger.info(
        "Generating with %s/%s (%d chunks, template=%s)",
        provider,
        model,
        len(included_chunks),
        prompt_template,
